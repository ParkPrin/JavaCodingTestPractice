package me.parkprin.codingtest.programmers.leveltest.quiz2;
...
class Solution {
    ...
    public String solution(String m, String[] musicinfos) {
        // m을 크기를 시간을 으로 나타냄
        String answer = "";
        int timeSize = m.length();

        for (int i =1; i < m.length(); i++){
            String prefix = m.substring(0, i);
            String[] mSplit = m.split(prefix);
            if (mSplit.length == 0) m = prefix;
        }

        char[] mCharacters = m.toCharArray();
        ...
        for (String musicinfo: musicinfos){
            ...
            String mCode= musicinfoSplit[3];
            MusicInfo musicInfo = new MusicInfo(startTime, endTime, title, mCode);
            boolean isExist = true;

            if (mCode.indexOf(mCharacters[0]) != -1){
                String prefix = mCode.substring(mCode.indexOf(mCharacters[0]));
                String surfix = mCode.substring(0, mCode.indexOf(mCharacters[0]));
                mCode = prefix +surfix;
            } else isExist = false;

            if (isExist && mCode.length() > mCharacters.length) {
                for (int i =0; i < mCharacters.length; i++){
                    if (mCode.charAt(i) != mCharacters[i]) isExist = false;
                }
            }

            if (isExist && mCode.length() > mCharacters.length) {
                if (mCode.charAt(mCharacters.length) == '#') isExist = false;
            }

            if (isExist) musicInfos.add(musicInfo);
        }

        if (musicInfos.size() == 1) answer = musicInfos.get(0).title;
        else {
            int maxTime = timeSize;
            String title = "";
            for (MusicInfo musicInfo: musicInfos){
                if (maxTime == 0){
                    maxTime = musicInfo.getTime();
                    title = musicInfo.getTitle();
                }  else if (musicInfo.getTime() > maxTime) {
                    maxTime = musicInfo.getTime();
                    title = musicInfo.getTitle();
                }
            }
    ...
    public class MusicInfo{
        ...
        public MusicInfo(String startTime, String endTime, String title, String mCode){
        ...
        public int getTime(){
            ...
            try {
                ...
                long minuate = diff / 60000;
                time = Long.valueOf(minuate).intValue();
            } catch (Exception e){}
